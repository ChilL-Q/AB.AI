        ...
        <CardContent className="p-0">
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center space-y-3">
              <p className="text-muted-foreground">
                {search ? "По запросу ничего не найдено" : "Клиентов пока нет"}
              </p>
              {!search && (
                ...
                <TableBody>
                  {clients.map((c) => (
                    <TableRow key={c.id} className="cursor-pointer">
                      ...
                      <TableCell className="text-right tabular-nums">{formatMoney(c.total_spent)}</TableCell>
                      <TableCell className="text-muted-foreground">{formatDate(c.last_visit_at)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
